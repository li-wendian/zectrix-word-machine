...
void ChargeStatus::Init(gpio_num_t detect_gpio, gpio_num_t full_gpio, int64_t now_ms) {
    ...
    cfg.pin_bit_mask = 1ULL << detect_gpio_;
    cfg.pull_up_en = CHARGE_DETECT_CHARGING_LEVEL == 0 ? GPIO_PULLUP_ENABLE : GPIO_PULLUP_DISABLE;
    cfg.pull_down_en = CHARGE_DETECT_CHARGING_LEVEL == 0 ? GPIO_PULLDOWN_DISABLE : GPIO_PULLDOWN_ENABLE;
    ESP_ERROR_CHECK_WITHOUT_ABORT(gpio_config(&cfg));

    cfg.pin_bit_mask = 1ULL << full_gpio_;
...
void ChargeStatus::Tick(int64_t now_ms) {
    ...
    const bool recent_power_signal = (last_power_present_ms_ >= 0) &&
    ...
    const bool alt_seen = recent_power_signal &&
    ...
    const bool power_present = detect_stable || full_stable || no_battery;
