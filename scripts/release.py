...
def _board_config_candidate(board_type: str) -> str:
    token = re.sub(r"[^A-Za-z0-9]+", "_", board_type).upper().strip("_")
    return f"CONFIG_BOARD_TYPE_{token}"
...
def _find_board_config(board_type: str) -> Optional[str]:
    ...
    for config, b_type in _parse_board_config_map().items():
        if b_type == board_type:
            return config
    candidate = _board_config_candidate(board_type)
    kconfig_file = Path("main/Kconfig.projbuild")
    if not kconfig_file.exists():
        return candidate
    kconfig_symbol = candidate.removeprefix("CONFIG_")
    if re.search(rf"^\s*config\s+{re.escape(kconfig_symbol)}\s*$", kconfig_file.read_text(encoding="utf-8"), re.MULTILINE):
        return candidate
    return None
...
def _board_type_exists(board_type: str) -> bool:
    ...
    compile_definition = f'BOARD_TYPE=\\"{board_type}\\"'
    return pattern in text or compile_definition in text or (_BOARDS_DIR / board_type).is_dir()
...
def release(board_type: str, config_filename: str = "config.json", *, filter_name: Optional[str] = None) -> None:
    ...
    for build in builds:
        ...
        if board_type_config:
            ...
            print(f"[WARN] 未找到 {board_type} 对应的 CONFIG_BOARD_TYPE，继续使用显式 BOARD_TYPE 编译")
