...
APP_BIN_NAME = os.environ.get("FIRMWARE_APP_BIN_NAME", "zectrix_word_machine.bin")
...
def read_binary(dir_path):
    ...
    # extract bin file
    bin_path = os.path.join(dir_path, APP_BIN_NAME)
    ...
    for name in os.listdir(release_dir):
        if name.startswith("v") and name.endswith(".zip"):
            ...
            if not os.path.exists(info_path):
                ...
                info["tag"] = tag
                info["url"] = os.path.join(os.environ['OSS_BUCKET_URL'], target_dir, APP_BIN_NAME)
